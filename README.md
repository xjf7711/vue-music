...
>上部分是一个轮播图组件，使用第三方库 better-scroll 辅助实现，使用ajax方式抓取 QQ音乐(移动端)数据   

>下部分是一个歌单推荐列表，使用 axios + proxy 代理后端请求，绕过主机限制 (伪造 headers)，抓取 QQ音乐(PC端)数据
...
>左侧是一个歌手列表，使用 ajax 抓取 QQ音乐(PC端)歌手数据并重组 JSON 数据结构
...
>歌词的爬取利用 proxy 代理后端请求，伪造 headers 来实现，先将axio获取json 格式歌词，再使用第三方库 js-base64 进行 Base64 解码操作，最后再使用第三方库 lyric-parser对歌词进行格式化
...
>此应用的全部数据来自 QQ音乐，推荐页的歌单列表及歌词是利用 axios 结合 proxy 代理后端请求抓取的。
